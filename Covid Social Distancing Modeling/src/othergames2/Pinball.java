...
public class Pinball extends JPanel {
	...
	public Pinball() {
		...
		for (int k = 0; k < balls.length; k++) {
			...
			for (double i = 0; i < 2 * Math.PI; i += 2 * Math.PI / sides) {
				count++;
				lines[plan + count][0] = px + (int) (radius * Math.cos(i));
				lines[plan + count][1] = py + (int) (radius * Math.sin(i));
				lines[plan + count][2] = px + (int) (radius * Math.cos(i - 2 * Math.PI / sides));
				lines[plan + count][3] = py + (int) (radius * Math.sin(i - 2 * Math.PI / sides));
			}
		}
	...
	public static class MainForGraphics1 {
